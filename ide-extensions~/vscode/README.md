...
### [1.12.1] - 2026-09-12
- Change: new extension icon. The previous mark placed Unity's logo at the centre of the atom, which the Unity Asset Store does not permit outside packages published by Unity themselves; the same rule applies to marketplace listings. It is a game controller now. Both the marketplace icon and the `.uitkx` file-type icon were replaced. No functional change - the language server, grammar and diagnostics are byte-identical to 1.12.0.
...
- Breaking: an export named with a C# reserved keyword is now an error — `UITKX2114`. An export name is emitted VERBATIM as a C# class or member name, so `export Style default = …` produced uncompilable generated code and a storm of `CS` errors pointing at a file the author never wrote. Components were already shielded by the PascalCase rule (every C# keyword is lowercase), so the reachable hole was the camelCase kinds: style, utils and value exports. The comparison is ORDINAL, which is the point — `new` is reserved, `New` is a perfectly legal component name and stays one; contextual keywords (`value`, `var`, `record`) are legal identifiers and are deliberately absent. No valid file changes behaviour: code this rejects did not compile before either. The reserved-keyword table is now shared by the parser, the namespace sanitizer and the RUITK Builder's create/rename/add-prop gestures, so a name can never be accepted by one layer and rejected by another. SG suite 1915/1915, LSP suite 185/185.
