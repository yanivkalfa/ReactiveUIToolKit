...
namespace UitkxLanguageServer.Roslyn
{
    ...
    public sealed class RoslynHost : IDisposable
    {
        ...
        )
        {
            ...
            // Polyfill stubs are only needed when the real ReactiveUITK assembly
            // is NOT among the metadata references (e.g. Unity project not yet
            // compiled).  When the real DLL is loaded, event types and delegates
            // come from metadata — both delegate coloring and lambda binding
            // work correctly.  Having BOTH source stubs and the real DLL in the
            // same compilation causes CS0436 conflicts that break nested lambda
            // binding (e.g. Func<int,int,PointerEventHandler> = ... => _ => {}).
            bool needsPolyfill = !Array.Exists(refs, r =>
                r.Display != null
                && r.Display.IndexOf("ReactiveUITK", StringComparison.OrdinalIgnoreCase) >= 0);
