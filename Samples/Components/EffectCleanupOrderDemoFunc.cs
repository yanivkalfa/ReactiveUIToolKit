...
namespace ReactiveUITK.Samples.FunctionalComponents
{
    ...
    public static class EffectCleanupOrderDemoFunc
    {
        ...
        private static class EffectPanelFunc
        {
            ...
            {
                string label =
                    props != null && props.TryGetValue("label", out var l) && l is string ls
                        ? ls
                        : "Panel";
                int generation =
                    props != null && props.TryGetValue("generation", out var g) && g is int gi
                        ? gi
                        : 0;
                var log =
                    props != null && props.TryGetValue("log", out var logObj) && logObj is LogRef lr
                        ? lr
                        : null;
