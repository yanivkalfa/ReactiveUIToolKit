...
namespace ReactiveUITK.Samples.Shared
{
    public static class SharedListViewRow
    {
        public static VirtualNode Render(Dictionary<string, object> props, IReadOnlyList<VirtualNode> children)
        {
            ...
            string key = rowItem != null ? ($"shared-row-{rowItem.Id}-{index}") : ($"shared-row-missing-{index}");
            return V.VisualElement(new Style { (FlexDirection, "row"), (AlignItems, "center") }, key: key,
