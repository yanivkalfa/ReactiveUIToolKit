...
namespace ReactiveUITK.Core.Animation
{
    ...
    internal static class Animator
    {
        ...
        private static readonly ConditionalWeakTable<VisualElement, Dictionary<string, AnimationHandle>> active = new();

        public static List<AnimationHandle> PlayTracks(VisualElement ve, IReadOnlyList<AnimateTrack> tracks)
        {
        ...
        {
            if (ve == null || track == null) return null;
            ...
            var map = active.GetValue(ve, _ => new Dictionary<string, AnimationHandle>(StringComparer.Ordinal));
            ...
            handle.item = ve.schedule.Execute(() =>
            {
                ...
                double now;
                try { now = Time.realtimeSinceStartupAsDouble; } catch { now = (double)Time.realtimeSinceStartup; }
                ...
                Apply(ve, prop, v);
                try { track.OnUpdate?.Invoke(eased); } catch { }
                ...
                if (!track.Loop)
                {
                    ...
                    if (cycleIndex >= totalCycles - 1 && cycleT >= 1f - 1e-4f)
                    {
                        ...
                        Apply(ve, prop, final);
                        try { track.OnComplete?.Invoke(); } catch { }
                        handle.Stop();
                    }
                }
            }).Every(16); // ~60 FPS
        ...
        private static object ReadCurrent(VisualElement ve, string prop)
        {
            switch (prop)
            {
                case "opacity":
                    {
                        float v = 1f;
                        try { v = ve.resolvedStyle.opacity; } catch { }
                        return v;
                    }
        ...
        private static void Apply(VisualElement ve, string prop, object value)
        {
            ...
            switch (prop)
            {
                ...
                case "width":
                {
                    if (TryToFloat(value, out var f)) ve.style.width = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "height":
                {
                    if (TryToFloat(value, out var f)) ve.style.height = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "minWidth":
                {
                    if (TryToFloat(value, out var f)) ve.style.minWidth = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "minHeight":
                {
                    if (TryToFloat(value, out var f)) ve.style.minHeight = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "maxWidth":
                {
                    if (TryToFloat(value, out var f)) ve.style.maxWidth = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "maxHeight":
                {
                    if (TryToFloat(value, out var f)) ve.style.maxHeight = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "marginLeft":
                {
                    if (TryToFloat(value, out var f)) ve.style.marginLeft = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "marginRight":
                {
                    if (TryToFloat(value, out var f)) ve.style.marginRight = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "marginTop":
                {
                    if (TryToFloat(value, out var f)) ve.style.marginTop = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "marginBottom":
                {
                    if (TryToFloat(value, out var f)) ve.style.marginBottom = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "paddingLeft":
                {
                    if (TryToFloat(value, out var f)) ve.style.paddingLeft = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "paddingRight":
                {
                    if (TryToFloat(value, out var f)) ve.style.paddingRight = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "paddingTop":
                {
                    if (TryToFloat(value, out var f)) ve.style.paddingTop = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "paddingBottom":
                {
                    if (TryToFloat(value, out var f)) ve.style.paddingBottom = new Length(f, LengthUnit.Pixel);
                    break;
                }
                case "borderLeftWidth":
                {
                    if (TryToFloat(value, out var f)) ve.style.borderLeftWidth = f;
                    break;
                }
                case "borderRightWidth":
                {
                    if (TryToFloat(value, out var f)) ve.style.borderRightWidth = f;
                    break;
                }
                case "borderTopWidth":
                {
                    if (TryToFloat(value, out var f)) ve.style.borderTopWidth = f;
                    break;
                }
                case "borderBottomWidth":
                {
                    if (TryToFloat(value, out var f)) ve.style.borderBottomWidth = f;
                    break;
                }
                case "letterSpacing":
                {
                    if (TryToFloat(value, out var f)) ve.style.letterSpacing = f;
                    break;
                }
                case "backgroundColor":
                {
                ...
                case "unityTextOutlineColor":
                {
                    ...
                    {
                        try { ve.style.unityTextOutlineColor = oc; } catch { }
                    }
                    break;
                }
                case "unityTextOutlineWidth":
                {
                    if (TryToFloat(value, out var f)) { try { ve.style.unityTextOutlineWidth = f; } catch { } }
                    break;
                }
                case "translateX":
                {
