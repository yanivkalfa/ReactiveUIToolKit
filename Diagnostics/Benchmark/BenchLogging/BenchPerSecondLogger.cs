    ...
    public static class BenchPerSecondLogger
    {
        ...
        )
        {
          ...
          _env = new BenchEnv
          {
              ...
              productName     = string.IsNullOrEmpty(ov.productName) ? Application.productName : ov.productName,
              platform        = string.IsNullOrEmpty(ov.platform)    ? Application.platform.ToString() : ov.platform,
              graphicsDevice  = string.IsNullOrEmpty(ov.graphicsDevice) ? SystemInfo.graphicsDeviceName : ov.graphicsDevice,
              deviceModel     = string.IsNullOrEmpty(ov.deviceModel) ? SystemInfo.deviceModel : ov.deviceModel,
              deviceName      = string.IsNullOrEmpty(ov.deviceName)  ? SystemInfo.deviceName : ov.deviceName,
        ...
        public static void SampleFrame(float dt)
        {
            ...
            _accFpsSum += fps;
            _accFpsCount++;
            if (fps < _accFpsMin) _accFpsMin = fps;
            if (fps > _accFpsMax) _accFpsMax = fps;
            ...
            var monoUsed = 0L;
#endif
            if (monoUsed > _accMonoUsedMax) _accMonoUsedMax = monoUsed;
            if (monoUsed > _monoUsedMaxScenario) _monoUsedMaxScenario = monoUsed;
...
#endif
            Debug.Log($"[Bench] Wrote scenario JSON: {path}  (bins={_bins.Count}, samples={_samplesTotal})");
        ...
        private static BenchScenarioSummary BuildScenarioSummary()
        {
            ...
            float fpsAvg = 0f, fpsMin = float.PositiveInfinity, fpsMax = float.NegativeInfinity;
            if (_pctlCount > 0)
            {
            ...
            return new BenchScenarioSummary
            {
                fpsAvg = 0, fpsP95 = 0, fpsMin = 0, fpsMax = 0,
                samplesTotal = 0,
                secondsTotal = _bins?.Count ?? 0,
                monoUsedMax = _monoUsedMaxScenario,
                gcCollectionsTotal = GC.CollectionCount(0) - _gcAtScenarioStart
        ...
        private static void FlushSecondBin(int sec)
        {
            if (_accFpsCount <= 0)
            {
                ...
                _bins.Add(new BenchPerSecondBucket
                {
                    sec = sec,
                    fpsAvg = 0,
                    fpsMin = 0,
