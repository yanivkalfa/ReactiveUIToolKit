...
namespace ReactiveUITK.Samples.FunctionalComponents
{
    ...
    public static class ContextBailoutDemoFunc
    {
        ...
        // ─────────────────────────────
        // Using EmptyProps.Instance (singleton) instead of a dict so the bailout
        // test still works: same reference each render → props-equal → bailout.
        ...
        )
        {
            ...
            return V.VisualElement(
                ...
                V.Func(
                    MiddleLayerRender,
                    EmptyProps.Instance // singleton ref — never changes → triggers bailout
                )
            );
        }
    }
